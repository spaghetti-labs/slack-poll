...
export async function createPoll({
  ...
  const allUsersIs_Bot = users.members.map((m) => m.is_bot); // Getting all users's is_bot parameters

  var verifiedUsers = [];
  var conversationMembers = [];

  for (let i = 0; i < allUsersIs_Bot.length; i++) {
    // Getting users which is not bot with query is_bot: false
    if (allUsersIs_Bot[i] == false) {
      verifiedUsers.push(allUsers[i]); // Adding results to verifiedUsers array
    }
  }
  verifiedUsers.shift(); // Removing USLACKBOT from array, because it is actually bot but API saying not bot
  ...
  const options = optionTexts.map((option) =>
    OptionEntity.create({
      id: v4(),
      text: option,
    })
  );
  const voteRights = members.map((member) =>
    VoteRightEntity.create({
      id: v4(),
      userId: member,
    })
  );
